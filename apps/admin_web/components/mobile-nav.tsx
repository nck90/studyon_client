...
const TABS = [
  ...
  { href: "/ops", label: "운영", icon: ListChecks },
  { href: "/focus", label: "집중", icon: Focus },
  { href: "/parents", label: "CRM", icon: HeartHandshake },
];
...
export function MobileNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-[var(--nav-bottom-offset)] left-4 right-4 z-50 safe-bottom">
      <div
        className="relative bg-white/80 backdrop-blur-2xl backdrop-saturate-[1.8] rounded-[22px] border border-white/60 px-2 py-1.5"
        style={{
          boxShadow:
            "0 2px 32px rgba(108,92,231,0.10), 0 0 1px rgba(0,0,0,0.08), 0 8px 16px rgba(0,0,0,0.04)",
        }}
      >
        {/* Sliding indicator */}
        {activeIdx >= 0 && (
