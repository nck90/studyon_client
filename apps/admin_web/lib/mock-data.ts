...
export function getDashboard(): DashboardData {
  const checkedIn = SEATS.filter(
    (s) => s.status === "studying" || s.status === "onBreak",
  ).length;
  const emptySeats = SEATS.filter((s) => s.status === "empty").length;
  const studyingStudents = STUDENTS.filter(
    (s) => s.status === "studying" || s.status === "onBreak",
  );
  const totalStudyMinutes = studyingStudents.reduce(
    (acc, s) => acc + s.todayMinutes,
    0,
  );
