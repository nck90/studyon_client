...
import { getStudyOverview, getDirectorOverview } from "@/lib/api";
import type { StudyMetric, DirectorOverviewResponse } from "@/lib/api";
import {
  BarChart3,
  Clock,
  Users,
...
export default function AnalyticsPage() {
  ...
  const [overview, setOverview] = useState<DirectorOverviewResponse | null>(
    null,
  );
  ...
  const avgStudyPerStudent =
    uniqueStudents > 0 ? Math.round(totalStudyMinutes / uniqueStudents) : 0;

  // Daily aggregation for chart
  const dailyMap = new Map<
    string,
    { study: number; attendance: number; count: number }
  >();
  metrics.forEach((m) => {
  ...
  const studentMap = new Map<
    string,
    { name: string; minutes: number; sessions: number; streak: number }
  >();
  metrics.forEach((m) => {
    const existing = studentMap.get(m.studentId) ?? {
  ...
  const attTotal =
    attBreakdown.present + attBreakdown.late + attBreakdown.absent;
  const attRate =
    attTotal > 0
      ? Math.round(
          ((attBreakdown.present + attBreakdown.late) / attTotal) * 100,
        )
      : 0;
        ...
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mb-6 stagger-children">
            ...
            <div className="bg-white rounded-2xl p-5 border border-card-border card-shadow">
              ...
              <p className="text-2xl font-extrabold tabular-nums text-text-primary">
                {overview?.activeStudentCount ?? uniqueStudents}명
              </p>
            </div>
            <div className="bg-white rounded-2xl p-5 border border-card-border card-shadow">
              <div className="flex items-center justify-between mb-2">
              ...
              <p className="text-2xl font-extrabold tabular-nums text-text-primary">
                {formatMinutes(avgStudyPerStudent)}
              </p>
            </div>
          </div>
                ...
                <div className="flex items-end gap-2 h-40">
                  {dailyData.map((d, idx) => {
                    const heightPct = Math.round(
                      (d.studyHours / maxStudyHours) * 100,
                    );
                    return (
                      <div
                        key={idx}
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{
                            ...
                            minHeight: d.studyHours > 0 ? "6px" : "2px",
                          }}
                        />
              ...
              {attTotal === 0 ? (
                ...
                <div className="space-y-5">
                  {[
                    {
                      label: "출석",
                      count: attBreakdown.present,
                      color: "#00B894",
                      bg: "bg-accent-light",
                    },
                    {
                      label: "지각",
                      count: attBreakdown.late,
                      color: "#FDCB6E",
                      bg: "bg-warm-light",
                    },
                    {
                      label: "결석",
                      count: attBreakdown.absent,
                      color: "#E17055",
                      bg: "bg-hot-light",
                    },
                  ].map((item) => {
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            {[
              ...
              <div
                key={stat.label}
                className="bg-white rounded-2xl p-4 border border-card-border card-shadow text-center"
              >
                <div
            ...
            {studentRanking.length === 0 ? (
              ...
              <div className="space-y-0">
                {studentRanking.map((s, idx) => (
                  ...
                  >
                    <span className="w-6 text-center text-sm font-bold tabular-nums text-text-tertiary">
                      {idx + 1}
                    </span>
                    ...
                    <span className="flex-1 font-semibold text-text-primary text-sm">
                      {s.name}
                    </span>
