...
export default function SeatsPage() {
  ...
  const showGuide = !guideDismissed && !loading && seats.length <= 1;
  ...
  const handleAssign = async (studentId: string) => {
    ...
    setSubmitting(true);
    try {
      await assignSeat(selectedSeat.id, studentId, "FIXED");
      setModal(null);
      loadSeats();
    } catch {
      /* ignore */
      ...
      {showGuide && (
        <div className="mb-5 bg-primary-surface border border-primary/15 rounded-2xl p-4 relative">
          <button
            ...
            className="absolute top-3 right-3 text-primary/40 hover:text-primary"
          >
            <X size={16} />
          </button>
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <ol className="text-xs text-text-secondary space-y-1 list-decimal list-inside">
                <li>
                  <strong>좌석 추가</strong> — 우측 상단 &quot;좌석 추가&quot;
                  버튼으로 좌석을 만드세요. 여러 개를 한번에 추가할 수 있습니다.
                </li>
                <li>
                  <strong>학생 배정</strong> — 좌석을 클릭한 후 &quot;학생
                  배정&quot;으로 학생을 앉힐 수 있습니다.
                </li>
                <li>
                  <strong>잠금</strong> — 사용하지 않을 좌석은 잠금 처리할 수
                  있습니다.
                </li>
              </ol>
            </div>
      ...
      {total > 0 && (
        <div className="flex gap-3 mb-5 flex-wrap">
          {[
            ...
            <div
              key={s.label}
              className={`${s.bg} rounded-xl px-4 py-2.5 flex items-center gap-2`}
            >
          ...
          <div className="grid grid-cols-4 md:grid-cols-6 lg:grid-cols-8 gap-3">
            {seats.map((seat) => (
              ...
              >
                {/* Status dot */}
                <div
                  className={`absolute top-2.5 right-2.5 w-2 h-2 rounded-full ${statusDot[seat.status]}`}
                />

                {seat.status === "LOCKED" && (
                  <Lock size={16} className="text-hot/60 mb-1" />
                )}
                {seat.status !== "LOCKED" && (
                  <Armchair
                    size={16}
                    className={`mb-1 ${seat.status === "OCCUPIED" ? "text-primary/60" : "text-gray-300"}`}
                  />
                )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  >
                    {statusLabel[selectedSeat.status] ?? selectedSeat.status}
                  </span>
                </div>
              </div>
              <button
            ...
            <div className="bg-bg rounded-xl p-3 space-y-2 text-sm">
              ...
              {selectedSeat.currentStudent && (
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-text-primary">
                    {selectedSeat.currentStudent.user.name}
                  </span>
                </div>
              )}
            </div>
            ...
            <div className="border-t border-divider pt-4 space-y-2">
              {selectedSeat.status !== "OCCUPIED" && (
                <button
                  onClick={() => {
                    setModal(null);
                    setTimeout(() => openAssign(selectedSeat), 150);
                  }}
        ...
        >
          ...
          >
            ...
            <div className="bg-sky-light rounded-xl p-3 text-xs text-text-secondary">
              구역과 시작 번호를 입력하면 자동으로 이름이 생성됩니다. (예:
              A구역, 1번부터 → A-01, A-02...)
            </div>
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {Array.from({ length: Math.min(bulkCount, 8) }, (_, i) => {
                      ...
                      return (
                        <span
                          key={i}
                          className="px-2 py-1 bg-white rounded-lg text-xs font-medium text-text-secondary border border-card-border"
                        >
