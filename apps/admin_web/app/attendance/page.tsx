...
export default function AttendancePage() {
  ...
  const present = data.filter(
    (r) =>
      r.attendanceStatus === "PRESENT" || r.attendanceStatus === "CHECKED_IN",
  ).length;
      ...
      <div className="flex flex-wrap items-center gap-3 mb-6">
        ...
        {data.length > 0 && (
          <div className="flex gap-2">
            <span className="px-3 py-1.5 rounded-lg bg-accent-light text-accent text-xs font-semibold">
              출석 {present}
            </span>
            {late > 0 && (
              <span className="px-3 py-1.5 rounded-lg bg-warm-light text-warm text-xs font-semibold">
                지각 {late}
              </span>
            )}
            {absent > 0 && (
              <span className="px-3 py-1.5 rounded-lg bg-hot-light text-hot text-xs font-semibold">
                결석 {absent}
              </span>
            )}
          </div>
      ...
      {loading ? (
        ...
        <>
                ...
                {data.map((record, idx) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 tabular-nums text-text-secondary font-medium">
          ...
          <div className="md:hidden space-y-2">
            {data.map((record) => {
              const config =
                statusConfig[record.attendanceStatus] ?? statusConfig.PRESENT;
              return (
                <div
                  key={record.id}
                  className="bg-white rounded-2xl p-4 border border-card-border card-shadow press-scale"
                >
