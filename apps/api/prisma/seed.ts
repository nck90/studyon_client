import { BadgeType, PrismaClient, UserRole } from '@prisma/client';
...
async function main() {
  ...
  const adminUser = await prisma.user.upsert({
    ...
    update: { role: UserRole.DIRECTOR, name: '원장', status: 'ACTIVE' },
    create: {
  ...
  const studentUser = await prisma.user.upsert({
    ...
    update: { role: UserRole.STUDENT, name: '홍길동', status: 'ACTIVE' },
    create: {
