...
let AttendancesService = class AttendancesService {
        ...
        const result = await this.prisma.$transaction(async (tx) => {
            if (forceCloseStudySession) {
                ...
                for (const session of activeSessions) {
                    ...
                    if (openBreak) {
                        const breakMinutes = (0, date_util_1.diffMinutes)(openBreak.startedAt, now);
                    ...
                    const totalBreakMinutes = refreshedBreaks.reduce((sum, item) => sum + item.breakMinutes, 0);
    ...
    async listAdmin(date, classId, groupId, attendanceStatus) {
        const items = await this.prisma.attendance.findMany({
            where: {
                attendanceDate: date ? (0, date_util_1.dateOnly)(date) : undefined,
                attendanceStatus,
                student: {
